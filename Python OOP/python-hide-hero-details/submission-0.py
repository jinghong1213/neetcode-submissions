class SuperHero:
    ...
    def set_health(self, edit_health: int) -> None:
        if (edit_health > 100):
            print("You can't set the health to more than 100")
        elif(edit_health < 0):
            print("You can't set the health to less than 0")
        else:
            self.__health = edit_health
    
    def set_power_level(self, edit_power_level: int) -> None:
        if(edit_power_level > 10):
            print("You can't set the power level to more than 10")
        elif(edit_power_level < 1):
            print("You can't set the power level to less than 1")
        else:
            self.__power_level = edit_power_level
